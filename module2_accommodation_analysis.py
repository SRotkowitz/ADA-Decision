...
def assess_direct_threat(inp: Module2Input) -> DirectThreatOutput:
    ...
    if stereotype_based:
        factors_against.append(
            "Assessment was stereotype-based, not individualized — automatic procedural defeater (29 CFR § 1630.2(r))"
        )
...
def assess_accommodation_determination(
    ...
    1. is_covered_employer = false → no / not_covered_employer
    2. is_covered_individual = false → no / not_covered_individual
    3. has_qualifying_disability = no → no / not_disabled
    4. accommodation_rights_attach = false → no / not_disabled
    5. preliminary_qualified_without_accommodation = no → no / not_qualified
       NOTE: Uses preliminary field from Module 1, NOT wrapper composite.
    6. accommodation_request_valid = false → no / invalid_request
    7. undue_hardship_applies = yes → no / undue_hardship
    8. direct_threat_exists = yes → no / direct_threat
    9. All pass AND hardship=no AND threat=no → yes
    10. No short-circuit AND any upstream indeterminate → indeterminate
    ...
    # R041-SC5: Preliminary qualified without accommodation check
    # Uses preliminary_qualified_without_accommodation from Module 1 (not wrapper composite)
    if qi.preliminary_qualified_without_accommodation == "no":
        return AccommodationDeterminationOutput(
            accommodation_required="no",
            denial_basis="not_qualified",
        )

    # R041-SC6: Valid accommodation request check
    ...
    # R041-SC9: All pass — accommodation required
    if (
    ...
    # R041-SC10: No short-circuit fired but upstream indeterminate
    return AccommodationDeterminationOutput(
